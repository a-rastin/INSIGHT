...
function formatNumber(value: number): string {
  if (!Number.isFinite(value)) throw new TypeError("TABLE values must be finite numbers");
  return Object.is(value, -0) ? "-0" : String(value);
}

export function serializeXmlBif(file: XmlBifFile): string {
  if (file.version !== "0.3") throw new RangeError(`Unsupported XMLBIF version: ${file.version}`);
  const lines = ['<?xml version="1.0" encoding="UTF-8"?>', '<BIF VERSION="0.3">'];
  for (const network of file.networks) {
    ...
    for (const variable of network.variables) {
      ...
      for (const outcome of variable.outcomes) lines.push(element("      ", "OUTCOME", outcome));
    ...
    for (const definition of network.definitions) {
      ...
      for (const parent of definition.given) lines.push(element("      ", "GIVEN", parent));
      lines.push(element("      ", "TABLE", definition.table.map(formatNumber).join(" ")));
