...
function assertAxisIndex(axisIndex: number, axisCount: number, allowEnd = false): void {
  const upperBound = allowEnd ? axisCount : axisCount - 1;
  if (!Number.isSafeInteger(axisIndex) || axisIndex < 0 || axisIndex > upperBound) {
...
function assertPermutation(permutation: readonly number[], size: number): void {
  if (
    ...
    permutation.some((index) => !Number.isSafeInteger(index) || index < 0 || index >= size) ||
    new Set(permutation).size !== size
  )
    throw new RangeError("Invalid permutation");
}

function assertTensorShape(values: readonly number[], cardinalities: readonly number[]): void {
  const expectedLength = product(cardinalities);
  if (values.length !== expectedLength) {
    throw new RangeError(`Tensor has ${values.length} values; expected ${expectedLength}`);
  }
}
...
export function coordinatesToFlatIndex(
  ...
  return coordinates.reduce((flatIndex, coordinate, axis) => {
    if (!Number.isSafeInteger(coordinate) || coordinate < 0 || coordinate >= cardinalities[axis]) {
...
export function permuteAxes(
  ...
  const newCardinalities = axisPermutation.map((oldAxis) => oldCardinalities[oldAxis]);
  return values.map((_, newFlatIndex) => {
    const newCoordinates = flatIndexToCoordinates(newFlatIndex, newCardinalities);
...
export function insertAxis(
  ...
  return Array.from({ length: product(newCardinalities) }, (_, newFlatIndex) => {
    const oldCoordinates = flatIndexToCoordinates(newFlatIndex, newCardinalities);
...
export function removeAxisState(
  ...
  assertTensorShape(values, cardinalities);
  assertAxisIndex(axisIndex, cardinalities.length);
  if (cardinalities[axisIndex] <= 1) throw new RangeError("Cannot remove the only axis state");
  if (
    !Number.isSafeInteger(stateIndex) ||
    stateIndex < 0 ||
    stateIndex >= cardinalities[axisIndex]
...
export function removeAxisIfLossless(
  ...
  const result: number[] = [];
  for (let flatIndex = 0; flatIndex < product(remainingCardinalities); flatIndex += 1) {
    const coordinates = flatIndexToCoordinates(flatIndex, remainingCardinalities);
    ...
    for (let state = 1; state < removedCardinality; state += 1) {
      ...
      if (!equal(first, values[coordinatesToFlatIndex(coordinates, cardinalities)])) return null;
    }
    result.push(first);
  }
  return result;
