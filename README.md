...
- `apps/server/src/bn-model` owns Administrator-only XMLBIF artifact upload and immutable model-version reads. Uploads receive backend-assigned pathway versions, persist content-addressed artifacts, retain reproducible software diagnostics and graph projections, activate only software-compatible versions, and keep evidence, calibration, and clinical-review status separate from software validity.
...
- [`Bayesian-Engine/`](Bayesian-Engine/) contains the intact standalone Electron XMLBIF editor. Its environment-independent parser, validation, diagnostics, graph-projection, and node-inspection behavior has been adapted into `packages/bayes` and the read-only React BN Manager; production does not use Electron or its filesystem bridge.
