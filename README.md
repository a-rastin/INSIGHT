...
- `apps/server/src/medication` owns Administrator-governed immutable canonical medication catalogs and backend-internal MCP search and commit handlers. Search applies deterministic Unicode, case, punctuation, and whitespace normalization; every returned candidate set is persisted. Commit accepts only a candidate returned for the same execution, medication entry, and catalog version, while `null` records `UNKNOWN`. Mapping provenance pins raw text, candidate snapshot, selection, catalog, model, prompt/schema versions, and timestamps without Psychiatrist confirmation.
