...
- `apps/web` is the React/Vite browser entry point with role-owned navigation and the Psychiatrist-only shared Patient Registry, create, profile, assessment, Medical History, Bayesian processing, and DDI status workflow pages. The Bayesian view resumes durable progress after refresh, exposes deterministic routing, model/source/version/hash provenance, evidence/calibration/review status, accepted snapshot outputs, and typed failures while keeping raw model reasoning and internal identifiers hidden. It labels every CPT as LLM-generated patient-specific research values, distinguishes mathematical validity from calibration, and visibly blocks inactive, failed, or stale processing. The DDI view resumes durable progress after refresh, renders exact regimen and evidence provenance, keeps findings warning-only, shows one generic unknown-medication warning, and blocks continuation only for failed or stale checks.
...
No Encounter or visit entity exists. Treatment Plan clinician review and finalization are not implemented yet. The unified deployment includes local password authentication, Administrator account management, and Patient/Research Case persistence.
