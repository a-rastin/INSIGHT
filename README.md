...
- `apps/server/src/bn-model` owns Administrator-only XMLBIF artifact upload, validated editor-candidate creation, and immutable model-version reads. Uploads receive backend-assigned pathway versions and may activate after software checks; graph, outcome, CPT/raw-table, and XML edits always persist as new inactive `IMPORTED` candidates. Both paths use content-addressed artifacts, retain reproducible diagnostics and graph projections, and keep evidence, calibration, and clinical-review status separate from software validity.
