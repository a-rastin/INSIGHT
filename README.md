...
- **DDI knowledge:** Administrator-only import stores Medscape artifacts on the persistent volume with versioned manifests, deterministic parser/transform versions, evidence line references, and an append-only review lifecycle. Sources remain disabled until written permission plus legal and clinical approval references pass the activation gate.
...
- `apps/web` is the React/Vite browser entry point with role-owned navigation and the Psychiatrist-only shared Patient Registry, create, profile, assessment, Medical History, Bayesian processing, DDI status, and Primary Treatment Plan review pages. The Bayesian view resumes durable progress after refresh, exposes deterministic routing, model/source/version/hash provenance, evidence/calibration/review status, accepted snapshot outputs, and typed failures while keeping raw model reasoning and internal identifiers hidden. It labels every CPT as LLM-generated patient-specific research values, distinguishes mathematical validity from calibration, and visibly blocks inactive, failed, or stale processing. The DDI view resumes durable progress after refresh, renders exact regimen and evidence provenance, keeps findings warning-only, shows one generic unknown-medication warning, and blocks continuation only for failed or stale checks. Primary Plan review renders structured regimen, monitoring, rationale links to authorized source records, warnings, provenance, and generated-baseline differences; incomplete, failed, dependency-unavailable, job, and stale states never appear ready. It shows one generic AI-imputation notice only when required and preserves explicit Psychiatrist control wording.
...
- `apps/server/src/bn-model` owns Administrator-only XMLBIF artifact upload, validated editor-version creation, and immutable model-version reads. Every new passing version activates atomically for its pathway and supersedes the prior active version; failed versions remain rejected. Administrators can disable or roll back pathway versions for new executions, while immutable Research Case pathway pins preserve the exact version and hashes already selected. All lifecycle transitions remain attributable, and evidence, calibration, and clinical-review status stay visible without gating software activation.
- `apps/server/src/bn-model` also owns the approved versioned deterministic routing artifact. Its fail-closed evaluator accepts only structured demographics, presentation, assessment, comorbidity, medication-history, and current-regimen facts; requires exactly one rule per required route group and one active compatible model per selected pathway; and immutably records matched rules plus exact model versions. The initial vertical slice routes eligible structured cases only to `BN-Pharmacotherapy.xml`; model-facing `bn.get_routed_contracts` accepts no model or pathway selector.
...
- `apps/server/src/medication` owns Administrator-governed immutable canonical medication catalogs, backend-internal MCP search and commit handlers, and revision-pinned durable normalization jobs. Jobs receive only the bounded de-identified medication projection, automatically commit a returned identity or `UNKNOWN` without confirmation, resume from persisted model checkpoints, and reject stale Research Case revisions. Mapping provenance pins raw text, candidate snapshot, selection, catalog, model, prompt/schema versions, and timestamps.
- `apps/server/src/ddi` owns deterministic unordered-pair enumeration and the internal `ddi.evaluate_regimen` MCP handler. Primary checks evaluate current/current and current/proposed pairs and expose every implicated drug for automatic-plan exclusion; final rechecks evaluate the exact complete regimen and keep all findings warning-only. Successful immutable executions pin the exact regimen, evaluated pairs, active proven source set, findings, unknown omissions, workflow/input revisions, and evidence-line references. Missing, disabled, incompatible, or provenance-invalid sources fail closed.
- `apps/server/src/treatment-plan` owns version `1.0.0` structured Primary Treatment Plan validation and the internal `treatment_plan.submit_primary` MCP handler. It accepts only normalized proposed candidates not excluded by the pinned Primary DDI execution, validates every rationale/warning/source reference against server-owned execution provenance, and persists one mutable revisioned draft with exact input execution pins and the imputation-notice flag. It has no finalization operation.
...
- [`Bayesian-Engine/`](Bayesian-Engine/) contains the intact standalone Electron XMLBIF editor. Its environment-independent parser, validation, diagnostics, tensor transforms, and safe mutations have been adapted into `packages/bayes`; React BN Manager synchronizes graph, ordered outcome, CPT/raw-table, and XML source projections while preserving malformed drafts separately from the last valid model and saving only new immutable candidates. Production does not use Electron or its filesystem bridge.
...
No Encounter or visit entity exists. Primary Treatment Plan clinician review display is implemented; editing and finalization are not implemented yet. The unified deployment includes local password authentication, Administrator account management, and Patient/Research Case persistence.
...
Migration 22 adds revision-pinned medication-normalization runs and optional route/frequency fields for prior medication trials. Each run pins the bounded projection, model endpoint, medication catalog, Medical History revision, Research Case workflow revision, and input revision. Worker completion requires every current and prior entry to be `NORMALIZED` or `UNKNOWN`; accepted canonical identities are the server-owned downstream values, while stale runs cannot update replacement entries.

Migration 24 adds immutable exact-regimen DDI executions. Pair order and output order are deterministic, duplicate canonical medicines do not duplicate pairs, `UNKNOWN` entries contribute an explicit omitted-pair count without failing the execution, Primary findings exclude every participating drug from automatic generation, and Final findings remain warnings. Every known medicine must resolve to an active permission-, legal-, clinical-, parser-, transform-, and evidence-hash-proven source version.

Migration 28 adds immutable patient-specific CPT generation attempts and accepted snapshots. `bn.get_routed_contracts` exposes only routed model topology, ordered outcomes and parents, and required dimensions; base numerical CPTs are never returned. `bn.submit_cpt_snapshot` stores each raw response, rejects missing, extra, duplicate, reordered, malformed, non-finite, negative, or non-normalized tables without repair, and makes the third invalid attempt terminal. Accepted snapshots are reused only within one Research Case while canonical model input, routed base models, prompt, schema, endpoint, requested model, generation settings, and optional imputation snapshot reference produce the same dependency fingerprint.

Migration 29 adds mutable revisioned Primary Treatment Plan drafts. Every save pins schema version, complete structured payload, exact upstream execution references, Primary DDI execution, workflow/input revisions, model-agent execution, attribution, and whether the generic AI-imputation notice must be visible. MCP submission can create or revise this draft but cannot approve, finalize, or supersede a plan.

Migration 30 adds one persisted Research Case orchestration run and immutable stage-attempt history. `POST /api/v1/patients/{patientId}/research-case/orchestration` queues an idempotent server-owned command; the worker resumes from the persisted workflow state, accepts only revision- and dependency-pinned domain results, cancels stale input revisions, and reaches `CLINICIAN_REVIEW` only after every owning module reports success. Reusing an idempotency key returns the original job, while a new key safely reruns a failed state without deleting prior attempts or provenance.
...
- The legacy DDI archive has not been imported under the governed manifest contract and still lacks the Medscape permission, legal approval, and clinical approval records required for activation.
