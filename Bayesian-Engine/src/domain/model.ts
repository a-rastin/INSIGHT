...
export function parentConfigurationCount(
  ...
  for (const parentName of definition.given) {
    const parentCardinality = cardinality(network, parentName);
    if (parentCardinality === undefined) return undefined;
    count *= parentCardinality;
  }
...
export function tableAxisNames(
  ...
  return child.type === "utility"
    ? [...definition.given]
    : [...definition.given, definition.for];
}
...
export function tableAxisCardinalities(
  ...
  const axisNames = tableAxisNames(network, definition);
  if (!axisNames) return undefined;

  const cardinalities: number[] = [];
  for (const axisName of axisNames) {
    const axisCardinality = cardinality(network, axisName);
    if (axisCardinality === undefined) return undefined;
    cardinalities.push(axisCardinality);
  }
...
export function expectedTableLength(
  ...
  const cardinalities = tableAxisCardinalities(network, definition);

  return cardinalities?.reduce((length, value) => length * value, 1);
}
