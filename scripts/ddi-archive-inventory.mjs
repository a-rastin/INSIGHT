...
export async function buildDdiArchiveInventory() {
  ...
  const batch3ReportPayload = {
    schemaVersion: "insight.ddi-import-report.v1",
    contract: "INS-055",
    batch: 3,
    batchSha256: batch3.batchSha256,
    inventorySnapshotSha256: inventory.snapshotSha256,
    evaluatedPositions: batch3.entries.map(({ position }) => position),
    approvedEntries: 0,
    importedEntries: 0,
    parserVersion: null,
    transformVersion: null,
    ...
    conflictRecords: [],
    lifecycleRecords: [],
    omissions: batch3.entries.map(({ position, path, blockedReasons }) => ({
      position,
      path,
      reasons: blockedReasons,
    })),
  ...
  const batch3Report = {
    ...batch3ReportPayload,
    reportSha256: hashPayload(batch3ReportPayload),
  };
  const batch3ReviewPayload = {
    schemaVersion: "insight.ddi-import-review.v1",
    contract: "INS-055",
    batch: 3,
  ...
  const batch3Review = {
    ...batch3ReviewPayload,
    reviewSha256: hashPayload(batch3ReviewPayload),
  };
  const batch4 = batches[3];
  const batch4ReportPayload = {
    schemaVersion: "insight.ddi-import-report.v1",
    contract: "INS-054",
    batch: 4,
    batchSha256: batch4.batchSha256,
    inventorySnapshotSha256: inventory.snapshotSha256,
    evaluatedPositions: batch4.entries.map(({ position }) => position),
    approvedEntries: 0,
    importedEntries: 0,
    parserVersion: null,
    transformVersion: null,
    ...
    conflictRecords: [],
    lifecycleRecords: [],
    omissions: batch4.entries.map(({ position, path, blockedReasons }) => ({
      position,
      path,
      reasons: blockedReasons,
    })),
  ...
  const batch4Report = {
    ...batch4ReportPayload,
    reportSha256: hashPayload(batch4ReportPayload),
  };
  const batch4ReviewPayload = {
    schemaVersion: "insight.ddi-import-review.v1",
    contract: "INS-054",
    batch: 4,
...
`;
  return {
    ...
    "batch-3-report.json": json(batch3Report),
