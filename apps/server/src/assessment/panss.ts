...
export async function savePanssAssessment(
  ...
  return withTransaction(pool, async (client) => {
    ...
    if (bypassed) {
      await client.query(`DELETE FROM insight.panss_assessments WHERE research_case_id = $1`, [
        researchCase.id,
      ]);
      await recordAssessmentCommit(client, researchCase.id, "PANSS", status, actor, now);
      return materialize(
        {
          ...researchCase,
